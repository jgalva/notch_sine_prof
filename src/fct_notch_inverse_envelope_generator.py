def fct_notch_inverse_envelope_generator(x_array, y_array, x1, m1, x2, m2, ymax):
    import numpy as np
    ...
    if x1 <= x_vertex and x_vertex <= x2:
        ...
        for i in range(x1_idx, x2_idx):
            if y_array_return[i] > ymax:
                y_array_return[i] = ymax

        if ymax is not None:
            x_cap_left_idx = np.where(abs(y_array_return[x1_idx:x2_idx] - ymax) < 0.01)[
                0
            ][0]
            x_cap_right_idx = np.where(
                abs(y_array_return[x1_idx:x2_idx] - ymax) < 0.01
            )[0][-1]
